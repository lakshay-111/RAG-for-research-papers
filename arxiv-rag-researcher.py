...
# Initialize reranker for better retrieval quality
try:
    reranker = CrossEncoder('cross-encoder/qnli-distilroberta-base')
except Exception as e:
    print(f"Warning: Could not load reranker model: {e}. Reranking will be skipped.")
    reranker = None
...
def semantic_chunk_text(text: str, max_chunk_tokens: int = 512, overlap_tokens: int = 100) -> List[str]:
    """
    Split text into semantic chunks based on token count with overlap.
    ...
    for sentence in sentences:
        ...
        if current_tokens + sentence_tokens > max_chunk_tokens and current_chunk:
            ...
            # Start new chunk with overlap (last ~100 tokens)
            overlap_chunk = current_chunk
            while count_tokens(overlap_chunk) > overlap_tokens:
                overlap_chunk = " ".join(overlap_chunk.split()[1:])
...
def rerank_chunks(query: str, chunks: List[Dict]) -> List[Dict]:
    ...
    try:
        ...
        texts = [chunk['content'][:500] for chunk in chunks]  # Use first 500 chars
